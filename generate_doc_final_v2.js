...
const children = [
  ...
  h2("（1）邮箱泄露检测"),
  p("邮箱泄露检测模块支持单个邮箱和批量文件两种检测模式。用户输入邮箱地址后，系统会自动调用Have I Been Pwned等公开泄露数据库进行查询，判断该邮箱是否出现在已知的数据泄露事件中。检测结果以表格形式呈现，包含泄露来源、泄露日期和受影响数据类型。批量模式支持导入Excel或TXT文件，一次性检测多个邮箱，检测完成后可导出完整报告。该模块帮助用户第一时间掌握自身邮箱的安全状况，及时采取改密等措施。"),
  imgPara(`${outputDir}/ui_email.png`, 480, 361),
  imgCaption("图3  邮箱泄露检测界面"),

  h2("（2）密码泄露检测"),
  p("密码泄露检测模块用于验证用户密码是否已在互联网泄露事件中被公开。系统通过K-Anonymity算法将密码转换为匿名化哈希前缀进行查询，确保原始密码不会暴露给第三方服务。支持单密码输入和批量文件导入两种模式，检测结果会标注密码在泄露数据库中出现的次数，次数越高说明风险越大。该模块可与邮箱检测配合使用，形成账号+密码的双重安全检查。"),
  imgPara(`${outputDir}/ui_password.png`, 480, 361),
  imgCaption("图4  密码泄露检测界面"),

  h2("（3）密码工具"),
  p("密码工具模块集成了随机密码生成和AI智能密码优化两项功能。随机密码生成器支持自定义长度（8-32位）和字符类型（大小写字母、数字、特殊符号），基于secrets模块确保密码的密码学安全性，生成后可一键复制到剪贴板。AI智能密码优化功能则调用DeepSeek大模型，将用户输入的个性化记忆短语（如诗句、歌词、纪念日）转换为符合安全规范的高强度密码变体，既保证了密码强度，又解决了强密码难以记忆的问题。"),
  imgPara(`${outputDir}/ui_pass_tools.png`, 480, 361),
  imgCaption("图5  密码工具界面（随机密码生成 + AI智能优化）"),

  h2("（4）PDF加解密"),
  p("PDF加解密模块为PDF文件提供便捷的密码保护功能。用户可选择单个PDF文件或批量导入文件夹，设置加密密码后生成受保护的PDF文件。加密采用PDF标准密码保护机制，兼容主流PDF阅读器。解密功能则用于解除已加密PDF文件的密码限制，方便用户在授权范围内进行文件共享和编辑。该模块适用于合同、报表等敏感文档的加密归档和传输场景。"),
  imgPara(`${outputDir}/ui_pdf.png`, 480, 361),
  imgCaption("图6  PDF加解密界面"),

  h2("（5）AES文本加解密"),
  p("AES文本加解密模块采用AES-256-GCM算法对纯文本内容进行加密保护。用户输入待加密的敏感文本（如服务器配置信息、数据库连接字符串），设置自定义密码后生成Base64编码的密文。解密时输入密文和对应密码即可还原原始内容。所有加解密操作均在本地完成，无需联网，确保数据不会离开本机。该模块适合在即时通讯工具中传递敏感信息时使用。"),
  imgPara(`${outputDir}/ui_aes.png`, 480, 361),
  imgCaption("图7  AES文本加解密界面"),

  h2("（6）图片隐写检测"),
  p("图片隐写检测模块用于检测图片文件中是否被嵌入了隐藏信息。系统通过分析图片的LSB（最低有效位）分布、文件结构异常和元数据特征，判断图片是否存在隐写痕迹。支持常见图片格式（PNG、JPG、BMP等），检测结果会显示疑似隐写的置信度和具体异常位置。该模块可用于识别通过图片传递秘密信息的潜在威胁，适用于网络安全检查和取证分析场景。"),
  imgPara(`${outputDir}/ui_stego.png`, 480, 361),
  imgCaption("图8  图片隐写检测界面"),

  h2("（7）二维码安全解析"),
  p("二维码安全解析模块帮助用户在扫码前预先识别二维码内容，避免扫描到恶意链接。用户可通过截图粘贴或文件导入的方式加载二维码图片，系统会自动解码并展示二维码中包含的URL或文本内容。对于URL类型的二维码，系统会对目标域名进行安全风险提示，帮助用户判断是否存在钓鱼、诈骗等恶意行为。该模块有效填补了'先扫码后判断'的安全空白。"),
  imgPara(`${outputDir}/ui_qr.png`, 480, 361),
  imgCaption("图9  二维码安全解析界面"),

  h2("（8）本机端口快照"),
  p("本机端口快照模块用于快速扫描和记录当前系统的网络端口开放情况。一键扫描即可列出所有处于监听状态的TCP/UDP端口、对应的进程名称和PID。支持将扫描结果导出为文本报告，便于与历史快照进行比对，及时发现异常端口开放或可疑进程。该模块是排查后门程序、恶意软件驻留和未经授权服务运行的有效工具。"),
  imgPara(`${outputDir}/ui_port.png`, 480, 361),
  imgCaption("图10  本机端口快照界面"),

  h2("（9）剪贴板安全哨兵"),
  p("剪贴板安全哨兵模块实时监控系统的剪贴板内容，识别并拦截敏感信息的意外外泄。当检测到剪贴板中包含身份证号、手机号、银行卡号、邮箱地址、密码等敏感数据时，系统会立即发出警告，并根据用户设置自动清空剪贴板或仅进行提示。支持预览当前剪贴板内容、手动清空和自动保护模式切换。该模块有效防止了因复制粘贴操作导致的敏感信息泄露。"),
  imgPara(`${outputDir}/ui_clipboard.png`, 480, 361),
  imgCaption("图11  剪贴板安全哨兵界面"),

  h2("（10）文件安全粉碎"),
  p("文件安全粉碎模块提供符合安全规范的数据删除功能，确保被删除文件无法通过常规恢复工具还原。支持选择单个文件或批量导入文件夹进行粉碎处理。用户可选择覆写次数（1次、3次、7次）和覆写模式（随机数据或0x00），次数越多安全性越高。系统会在粉碎完成后生成操作报告，记录处理的文件列表和采用的粉碎策略。该模块适用于离职调岗、设备报废等场景下的敏感数据清除。"),
  imgPara(`${outputDir}/ui_shred.png`, 480, 361),
  imgCaption("图12  文件安全粉碎界面"),

  h2("（11）系统设置"),
  p("系统设置模块用于配置DeepSeek AI服务的相关参数，包括API密钥、模型选择和代理设置。用户可在此输入自己的DeepSeek API密钥，选择适合的模型版本（如deepseek-chat或deepseek-coder），并配置HTTP代理以适配不同的网络环境。设置完成后，AI智能密码优化功能即可正常调用云端大模型服务。所有配置信息保存在本地JSON文件中，不会上传至任何第三方服务器。"),
...
Packer.toBuffer(doc).then(buffer => {
  fs.writeFileSync("d:/PythonProject/密码泄露/LeakGuard/在岗技术革新申报书_铁密_v3.docx", buffer);
  console.log("文档已生成：d:/PythonProject/密码泄露/LeakGuard/在岗技术革新申报书_铁密_v3.docx");
});
